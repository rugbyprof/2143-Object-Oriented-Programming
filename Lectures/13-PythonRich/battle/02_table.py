...
class Player:
    ...
    def __str__(self):
        return f"{self.name} {self.health}, {self.attack_types}"
...
class LiveData:
    ...
    def init_table(self) -> Table:
        """Make a new table."""
        self.table = Table()
        self.table.add_column("Attacker",justify="left", style="red")
        self.table.add_column("Type",justify="center", style="yellow")
        self.table.add_column("Defender",justify="right", style="green")
    ...
    def update_table(self) -> Table:
        ...
        self.table.add_row(
            f"{attacker.name}[{attacker.health}]",
            f"Attack: {attack} Value: {value}",
            f"{defender.name}[{defender.health}]",
        )
        return self.table
...
if __name__ == "__main__":
    ld = LiveData()
    with Live(ld.init_table(), refresh_per_second=4) as live:
        for _ in range(40):
            time.sleep(0.4)
            live.update(ld.update_table())