...
class Logger {
   public:
    ...
    static void printLastLine(WINDOW* win) {
        ...
        int maxh, maxw;  // height and width of window

        getmaxyx(win, maxh, maxw);

        log("gamelog xy", std::to_string(maxh) + "," + std::to_string(maxw));

        mvwprintw(win, maxh - 1, maxw / 2, "%-*s", maxw / 2, " ");  // blank out entire top line
        mvwprintw(win, maxh - 1, maxw / 2, lastLine.c_str());
        return;
    }
