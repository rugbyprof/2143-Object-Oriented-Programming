...
Image::Image(int width, int height)
    ...
    // _pixels is initialized to height rows, each with width default Colors.
    // Default Color() is (0, 0, 0) — pure black.
}
...
Image Image::load(const std::string& filename) {
    ...
    for (int row = 0; row < height; ++row) {
        for (int col = 0; col < width; ++col) {
            ...
            // stb gives us unsigned char [0,255]; Color stores int — safe conversion.
            // Color(r,g,b) clamps to [0,255] on construction (already in range here).
            img._pixels[row][col] = Color(data[idx + 0],   // R
                                          data[idx + 1],   // G
                                          data[idx + 2]);  // B
        }
    }
...
void Image::save(const std::string& filename, int jpegQuality) const {
    ...
    for (int row = 0; row < _height; ++row) {
        for (int col = 0; col < _width; ++col) {
            ...
            // Color channels are already clamped on construction, but in-place
            // filter arithmetic (e.g. Brighten) may push values out of range.
            buf[idx + 0] = static_cast<unsigned char>(std::clamp(_pixels[row][col][0], 0, 255));
            buf[idx + 1] = static_cast<unsigned char>(std::clamp(_pixels[row][col][1], 0, 255));
            buf[idx + 2] = static_cast<unsigned char>(std::clamp(_pixels[row][col][2], 0, 255));
        }
    }
...
// -----------------------------------------------------------------------
// Image::apply — non-destructive kernel convolution
//
// Returns a new Image; *this is unchanged.
//
// Algorithm:
//   1. Start with result = *this  (copy — border pixels stay unchanged)
//   2. half = k.size() / 2
//   3. For every interior pixel (half <= row < height-half, same for col):
//        For each channel ch in {0, 1, 2}:
//          sum = 0.0
//          For kr in {0 .. k.size()-1}:
//            For kc in {0 .. k.size()-1}:
//              sum += k.at(kr, kc) * _pixels[row + kr - half][col + kc - half][ch]
//          result[row][col][ch] = clamp(round(sum), 0, 255)
//   4. Return result.
//
// Hint: Color::operator[] lets you address channels as c[0], c[1], c[2].
//       But operator[] is a TODO — implement it in Color.cpp before here.
// -----------------------------------------------------------------------

Image Image::apply(const Kernel& k) const {
    // TODO: implement non-destructive convolution.
    //
    // Step 1: make a copy to preserve border pixels.
    //   Image result = *this;
    //
    // Step 2: compute the half-size (border width to skip).
    //   int half = k.size() / 2;
    //
    // Step 3: convolve interior pixels.
    //   for (int row = half; row < _height - half; ++row) {
    //       for (int col = half; col < _width - half; ++col) {
    //           for (int ch = 0; ch < 3; ++ch) {
    //               double sum = 0.0;
    //               for (int kr = 0; kr < k.size(); ++kr)
    //                   for (int kc = 0; kc < k.size(); ++kc)
    //                       sum += k.at(kr, kc) *
    //                              _pixels[row + kr - half][col + kc - half][ch];
    //               result[row][col][ch] = std::clamp(
    ...
    (void)k;          // remove this when implementing
