...
int main(int argc, char **argv)
{
    ...
    vector<int> A{ilist};         // initialized using initializer_list<int> constructor
    ...
    list<int> Clst = {5, 7, 9, 10}; // also initializes a stl linked list as well.
    ...
    // Iterators.  STL has a type called iterator that is used to traverse the elements
    // of a stl container.
    list<int>::iterator itor; // declares an iterator called itor that points to an element
                              // in list<int>.
    // Iterators can be thought of as a class that encapsulates pointers to the item.
    // STL types have two methods called begin() and end() among others.
    //      type.begin(): returns the itor that points to the first item in the container
    //      type.end(): points to one after the last item in the container.
    //      itor++: overloaded operator that increments the itor to point to the next item.
    ...
    // Note that they both have the SAME interface even though one is a list and the other
    // is a vector.  Works the way on most of the stl containers.

    // C++11 has a new mechanism called auto.  This is very nice.  It will automatically
    ...
    cout << endl;
    // Note that the above only works for stl containers.  If you want it to work
    // for say your own class you would need to add iterators and begin() and end()
    // to your class.  We might attemp this late in the semester.
